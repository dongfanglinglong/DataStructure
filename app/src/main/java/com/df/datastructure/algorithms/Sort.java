 ...
 * 选择排序  平均O(n^2)         最好O(n^2)    最坏O(n^2)   辅助空间O(1)         不稳     n较小时好（简单）
 ...
 * 快速排序  平均O(nlgn)        最好O(nlgn)   最坏O(n^2)   辅助空间O(lgn)~O(n)  不稳     n较大时好,基本有序反而不好（较复杂）
 * 堆 排 序  平均O(nlgn)        最好O(nlgn)   最坏O(nlgn)  辅助空间O(1)         不稳     n较大时好（较复杂）
 * </pre>
 * <p>
...
public class Sort {
    ...
    // 冒泡排序		平均O(n^2)			最好O(n)		最坏O(n^2)	辅助空间O(1)			稳定		n较小时好（简单）
    // 简单选择排序	平均O(n^2)			最好O(n^2)	最坏O(n^2)	辅助空间O(1)			不稳		n较小时好（简单）
    // 直接插入排序	平均O(n^2)			最好O(n)		最坏O(n^2)	辅助空间O(1)			稳定		基本有序时好（简答）
    // 希尔排序		平均O(nlgn)~O(n^2)	最好O(n^1.3)	最坏O(n^2)	辅助空间O(1)			不稳		n较小时好（简单）
    // 归并排序		平均O(nlgn)			最好O(nlgn)	最坏O(nlgn)	辅助空间O(n)			稳定		n较大时好（较复杂）
    // 快速排序		平均O(nlgn)			最好O(nlgn)	最坏O(n^2)	辅助空间O(lgn)~O(n)	不稳		n较大时好,基本有序反而不好（较复杂）
    // 堆排序		平均O(nlgn)			最好O(nlgn)	最坏O(nlgn)	辅助空间O(1)			不稳		n较大时好（较复杂）
